...
bool run_command(ConnClientEntry& client,const string& src_str){
  ...
  deque<UnixPipe>& pipe_pool = client.pipe_pool;
  ...
  // Env related
    ...
    if(!name.empty()){
      *client.p_fout << name << '=' << client.env.get_value(name) << endl;
    }
  }
    ...
    while(regex_search(cut_string,match,regex("(.+?)($|(\\| )|(([|!]\\d+\\s*)+))"))){
      ...
      if(suffix=="|"){
        ...
        if(suffix.size() >= 2){
          ...
          if(regex_search(suffix,match,regex("\\|(\\d+)"))){
            ...
            stdout_pipe = client.create_numbered_pipe(stdout_pipe_count);
          }
          // stderr
          if(regex_search(suffix,match,regex("!(\\d+)"))){
            stderr_pipe_count = stoi(match[1]);
            stderr_pipe = client.create_numbered_pipe(stderr_pipe_count);
          }
      ...
      if((pid = fork())>0){
        ...
        if(!result.empty()){
          // error
          if(result=="unknown"){
            *client.p_fout << "Unknown command: [" << src_cmd_line << "]." << endl;
          }
          else if(result=="pipe existed"){ // write pipe existed
            *client.p_fout << "*** Error: public pipe #" << pub_write_pipe <<" already exists. ***" << endl;
          }
          else if(result=="pipe not exist"){ // read pipe not existed
            *client.p_fout << "*** Error: public pipe #" << pub_read_pipe <<" does not exist yet. *** " << endl;
          }
          // restore pipe
          if(first){
          ...
          client.record_pipe_info(stdout_pipe_count,stdout_pipe);
          client.record_pipe_info(stderr_pipe_count,stderr_pipe);
          if(pub_write_pipe >= 0){
            stringstream ss;
            ss << "*** " << client.nick <<" (#" << client.id <<") just piped '" << str << "' ***\n";
            client.p_mgmt->send_broadcast_message(ss.str());
          }
          if(pub_read_pipe >= 0){
            stringstream ss;
            ss << "*** " << client.nick <<" (#" << client.id <<") just received via '" << str << "' ***\n";
            client.p_mgmt->send_broadcast_message(ss.str());
          }
        }
        // close error pipe
        ...
        client.env.apply_all();
        // child
        fd_reopen(FD_STDIN,client.fd);
        fd_reopen(FD_STDOUT,client.fd);
        fd_reopen(FD_STDERR,client.fd);
        ...
        if(first){
          if(pub_read_pipe >= 0){
            ...
            client.close_pipe_larger_than(0);
            ...
            client.close_pipe_larger_than(1);
